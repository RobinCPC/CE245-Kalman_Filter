...
p22_Rec(1)=P(2,2);     % <- 1 corresponds to t=2
sumErr=0;
sumErrW=0;
sumLogErr=0;
for t=3:length(y),
  ...
  p22_Rec(t-1)=P(2,2);     %<- corresponds to t after update
end
...
subplot(2,3,4),plot(xcorr_res1),title('r11'),ylabel('Model 2');
xcorr_res2=xcorr(res2_Rec,res2_Rec);
subplot(2,3,5),plot(xcorr_res2),title('r22');
xcorr_res12=xcorr(res1_Rec,res2_Rec);
subplot(2,3,6),plot(xcorr_res12),title('r12');
